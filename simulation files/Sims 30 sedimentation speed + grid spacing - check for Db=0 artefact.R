...
load("04 CSFe Steady Db~xL=0 KFIS=696 1.Rdata")

# Initialisation simulation type 

sim.info$index <- 1
sim.info$code <- "30 CSFe Steady v=0.5"
...
sim.info$index <- 1
sim.info$code <- "30 CSFe Transient v=0.1"
sim.info$name <- paste(sim.info$code,sim.info$index)
...
sim.info$index <- 1
sim.info$code <- "30 CSFe Steady v=0.1"
sim.info$name <- paste(sim.info$code,sim.info$index)
...
sim.info$index <- 1
sim.info$code <- "30 CSFe Transient v=0.5"
sim.info$name <- paste(sim.info$code,sim.info$index)
